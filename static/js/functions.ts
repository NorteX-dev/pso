export function ackleyFunction(x: number, y: number, optimum: number) {
...
export function boothFunction(x: number, y: number, optimum: number) {
	let p1 = Math.pow(x + 2 * y - 7, 2);
	let p2 = Math.pow(2 * x + y - 5, 2);
	return p1 + p2 + optimum;
}

export function threeHumpCamelFunction(x: number, y: number, optimum: number) {
	let p1 = 2 * x * x;
